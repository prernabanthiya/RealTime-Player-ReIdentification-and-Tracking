...
# Output video path with timestamp (avoid overwrite)
OUTPUT_VIDEO_PATH = rf'output_tracked_{int(time.time())}.mp4'

# DeepSort tracker params
MAX_AGE = 60        # How long to keep lost tracks before deletion (higher = better re-ID but slower to remove ghosts)
N_INIT = 3          # Frames needed to confirm a track (reduce false positives)
MAX_COSINE_DISTANCE = 0.2  # Appearance similarity threshold (lower = stricter)
...
while True:
    ...
    # Filter detections to 'player' class only (assuming class 'player' exists in model.names)
    dets_for_tracker = []
    for det in detections:
        ...
        if class_name == 'player':
            bbox = [x1, y1, x2 - x1, y2 - y1]  # format DeepSort expects: [x, y, w, h]
    ...
    for track in tracks:
        ...
        if track.time_since_update > 0:
            # Skip tracks not detected in current frame to avoid ghost boxes
            continue
    ...
    # Optional: Display frame in window, press 'q' to quit early
